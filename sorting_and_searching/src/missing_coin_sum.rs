...
fn testcase(reader:&mut BufReader<StdinLock<'static>> , writer: &mut BufWriter<StdoutLock<'static>>) {
    ...
    arr.sort();
    let mut ans:i64 = 1;
    // ans -> i can make a sum upto ans-1
    // check if i can make a sum of ans+coin
    for i in arr{
        if i as i64 <= ans {
            // i can make a sum of 1 to (ans + i )- 1 and i have to check for (ans + i) next
            /*
            ans +i  cannot be possible if i > ans because the min sum possible if we inculde i ans + (i - ans)
            if we dont include i, max sum possible will be ans - 1
             */ 
            ans += i as i64
        }
        else {
