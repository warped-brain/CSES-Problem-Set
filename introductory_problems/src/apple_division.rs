...
fn helper(i:usize, list:&Vec<i64>, sum:i64, k: i64, minDiff:&mut i64){
    if i == list.len(){
        return;
    }
    *minDiff = min(*minDiff, ((k - sum)-(sum)).abs());
    helper(i+1, list, sum + list[i], k, minDiff);
    helper(i+1, list, sum, k, minDiff);
}
fn testcase(reader:&mut BufReader<StdinLock<'static>> , writer: &mut BufWriter<StdoutLock<'static>>) {
    let mut n = read_int(reader);
    let mut list:Vec<i64> = read_int_list(reader).iter().map(| f| *f as i64).collect();
    let k:i64 = list.iter().sum();
    let mut minDiff = k+1;
    helper(0, &list, 0, k, &mut minDiff);
    println!("{minDiff}");
}
