...
fn helper(n:i32, mut i: i32, list:&mut Vec<String>){
    if i == n+1 {
        return;
    }
    let mut k = 2_usize.pow((i-1) as u32);
    for j in k..2*k {
        list[j] = list[k-(j-k+1)].clone();
    }
    for j in 0..k {
        list[j].insert(0, '0');
    }
...
fn testcase(reader:&mut BufReader<StdinLock<'static>> , writer: &mut BufWriter<StdoutLock<'static>>) {
    ...
    let mut list = vec![String::from("0"); 2_usize.pow(n as u32)];
        list[0] = String::from("0");
        list[1] = String::from("1");
    if n == 1 {
        write_string_list(writer, list);
        return;
    }
    helper(n, 2, &mut list);
    write_string_list(writer, list);
