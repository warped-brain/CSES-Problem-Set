...
const TOWERS:i32 = 1^3^2;

fn helper(n:i32, from: i32, to :i32){
    if(n == 0){
        // println!("{from} {to}");
        return;
    };
    let intermediate = TOWERS^from^to;
    helper(n-1, from, intermediate);
    println!("{from} {to}");
    helper(n-1, intermediate, to);
    
}
fn testcase(reader:&mut BufReader<StdinLock<'static>> , writer: &mut BufWriter<StdoutLock<'static>>) {
    let mut n = read_int(reader);
    println!("{}", 2_i32.pow(n.try_into().unwrap()) -1);
    helper(n, 1, 3);
}
